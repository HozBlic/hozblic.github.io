...
function loadWrappedTab() {
    ...
    //get perk names
    // redo image getting, stone path not found
    //54 days , actually more, do last 100
    ...
    console.log(objInfoPlayer)
    var objInfoGameData = jsonBlocks['gamedata'];
    var objInfoGameDataFacts = jsonBlocks['gamedata']['t2_world_facts'];
    var objInfoGameStats = jsonBlocks['game_stats'];
    var arrTips = [];
    var arrTipsHtml = [];
    ...
    var $divInfoBlockWrap = getBlock(8, 6, true, null, true);
    ...
    // Name
    var $divInfoBlockWrap2 = getBlock(8, 2, true);
    ...
    $('#wrapped').append($divInfoBlockWrap);
    /* END: Basic stats */
    ...
    $('#wrapped').append($divInfoBlockWrapMiniGrid);
    ...
    $divInfoBlock.append(`<div class="info_horizontal">
                            ...
                            <div class="info_value calculate_spacing">${getFlexChildren(objInfoPlayer['recipe_unlocks'].length)}</div>
    ...
    $divInfoBlock.append(`<div class="info_horizontal">
                            ...
                            <div class="info_value calculate_spacing">${getFlexChildren(objInfoPlayer['recipes_created'].length)}</div>
                            <div class="info_desc calculate_spacing">${getFlexChildren('recipes')}</div>
                        </div>`);
    $($divInfoBlockWrap2).append($divInfoBlock);
    $($divInfoBlockWrapMiniGrid).append($divInfoBlockWrap2);
    ...
    Object.entries(objCharacters).forEach(([strCharKey, value]) => {
        ...
        if (strCharKey == 'seridia') {
            return;
        }
        var intDayLastSpoken = Math.round((intLastSpoken / 86400) + 1);
        objCharactersSpokenAgo[strCharKey] = intDay - intDayLastSpoken;
    ...
    var intDataAvailable = objInfoGameStats['end_of_day_balance'].length;
    var arrDaysLabels = [...Array(intDataAvailable).keys()].map(i => intDay - i).reverse();
    for (let i = 0; i < arrDaysLabels.length; i++) {
    ...
    $divInfoBlockWrap.append(`<div class="info_title">Stamina, HP and bedtime in the last 54 days</div>`);
    $($divInfoBlockWrap).append(`<div class="chart-container">
    ...
    $divInfoBlockWrap.append(`<div class="info_title">You have lost ${intLostCount} items across ${intLost} locations!</div>`);
    $($divInfoBlockWrap).append(`<div class="chart-container">
    ...
    $divTableWrap.append('<div class="info_desc">last 54 days</div>');
    ...
    $divInfoBlock.append(`<div class="info_horizontal">
                            ...
                            <div class="info_value calculate_spacing">${getFlexChildren(intSoldCount)}</div>
    ...
    Object.entries(objSoldSortedFirst).forEach(([strItemKey, intValue]) => {
        ...
        arrTips.push(strID);
        arrTipsHtml.push(createTip(strID, objItems[strItemKey], false, strItemKey));
    ...
    // Info prep - table and chart (sold last 54)
    ...
    var arrSoldCountPerDay = [...Array(intDataAvailable).keys()].fill(0);
    Object.entries(objItemsSoldPerDay).forEach(([key, value]) => {
        if (value.length) {
            ...
            value.forEach(function (item, index) {
                ...
                arrSoldCountPerDay[key] = arrSoldCountPerDay[key] + item['count']
            });
        }
    });
    ...
    Object.entries(objSoldSortedFirst).forEach(([strItemKey, intValue]) => {
        strID = 'most_sold_54_' + strItemKey;
        arrTips.push(strID);
        arrTipsHtml.push(createTip(strID, objItems[strItemKey], false, strItemKey));
        if (strItemKey in objItems) {
            strTableSold += `<tr>
    ...
    $divInfoBlockWrap.append(`<div class="info_title">Sold items in the last 54 days</div>`);
    $divInfoBlockWrap.append(`<div class="info_desc">You have sold ${intSoldCount} items, from which ${intSoldDistinct} are distinct</div>`);
    ...
    const arrLocationVisitCount = Object.values(objInfoGameStats['location_visits']);
    ...
    Object.entries(objInfoGameStats['location_visits']).forEach(([strLocationKey, intVisits]) => {
        if (strLocationKey in objLocations && intVisits > 0) {
            datasets.push({
                ...
                backgroundColor: `rgba(${parseInt(arrPalette[0].slice(1, 3), 16)}, ${parseInt(arrPalette[0].slice(3, 5), 16)}, ${parseInt(arrPalette[0].slice(5, 7), 16)}, ${getOpacity(intVisits)})`
            })
        }
    });
    ...
    $divInfoBlockWrap.append('<div class="info_desc">last 54 days</div>');
    $($divInfoBlockWrap).append(`<div class="chart-container">
...
function getBlock(intW, intH, bolOverflowVisible = false, bolHasTable = false, bolInnerGrid = false) {
...
const getOrCreateTooltip = (chart) => {
    ...
    if ($tooltipEl.length === 0) {
        $tooltipEl = $('<div class="chart-tooltip"></div>').css({
...
const externalTooltipItem = (context) => {
    ...
    // Set Text
    if (tooltip.body) {
        const intCount = tooltip.dataPoints[0].raw;
        const strItemKey = tooltip.dataPoints[0].dataset.label;

        let strItemName;
        if (strItemKey in objItems) {
            strItemName = `<img src="images/items/${strItemKey}.png" style="height:100%;">`;
        } else {
            strItemName = capitalizeFirstLetter(strItemKey.replaceAll('_', ' ')) + ': ';
        }

        $tooltipBody.append(strItemName).append(intCount);

        $tooltipEl.empty().append($tooltipBody);

        // Optionally, you can add more info here using jQuery if needed
    }
